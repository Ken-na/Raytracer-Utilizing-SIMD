...
bool GetMaterial(const Config &sceneFile, Material &currentMat)
{
	...
	{
        currentMat.type = Material::MaterialType::CHECKERBOARD;
	} 
	else if (materialType.compare("wood") == 0)
    {
        currentMat.type = Material::MaterialType::WOOD;
	}
	else if (materialType.compare("circles") == 0)
	{
		currentMat.type = Material::MaterialType::CIRCLES;
	}
	else
    { 
        // default
        currentMat.type = Material::MaterialType::GOURAUD;
    }
