...
bool isSphereIntersected(const Scene* scene, const Ray* r, const float t)
{
	...
	for (unsigned int i = 0; i < scene->numSpheresSIMD; ++i)
	{
		...
		Vector8 dist = pos - rStart;	// Vector dist = pos - r.start;
		
		__m256 Bs = dot(rDir, dist);	// float B = r.dir * dist;
		
		__m256 Ds = Bs * Bs - dot(dist, dist) + sizes * sizes;	// float D = B * B - dist * dist + size * size;

		// if D < 0, no intersection, so don't try and calculate the point of intersection
		__m256 DLessThanZeros = Ds < zeros;	//if (D < 0.0f) continue;

		// calculate both intersection times(/distances)
		__m256 sqrtDs = _mm256_sqrt_ps(Ds);
		__m256 t0s = Bs - sqrtDs;	//float t0 = B - sqrtf(D);
		__m256 t1s = Bs + sqrtDs;	//float t1 = B + sqrtf(D);
...
bool isCylinderIntersected(const Scene* scene, const Ray* r, float* t, Vector* normal, unsigned int* planeIndex)
{
	...
	for (unsigned int i = 0; i < scene->numCylSIMD; i++) {
	...
	Vector newNormal = { normals.xs.m256_f32[*planeIndex % 8], normals.ys.m256_f32[*planeIndex % 8], normals.zs.m256_f32[*planeIndex % 8] };
